...
Status: documentation-verified on 2026-07-21; hash-bound in
...
manual turn boundaries, static gateway function schema, omitted
...
| Provider | Frozen model / voice | PCM input -> output | Manual boundary | Per-turn HACC context authority |
|---|---|---:|---|---|
...
| xAI | `grok-voice-think-fast-1.0` / `ara` | 24 kHz -> 24 kHz | commit, then response | per-response instructions override session instructions |
...
- Gemini's setup-complete message does not echo the requested setup, so exact
  ...
  requires exact transport acknowledgement; the current xAI adapter does not.
...
- xAI: [Voice Agent API](https://docs.x.ai/developers/model-capabilities/audio/voice-agent) and [Voice overview and voice list](https://docs.x.ai/developers/model-capabilities/audio/voice).
