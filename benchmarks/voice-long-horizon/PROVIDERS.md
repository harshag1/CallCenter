...
| Primary LC4 turns | Disable VAD and explicitly commit audio/create response | Documented `server_vad`; provider speech stop automatically commits and starts the initial response; exactly one explicit continuation follows tool output | Disable automatic activity detection and send `activityStart`/`activityEnd` |
