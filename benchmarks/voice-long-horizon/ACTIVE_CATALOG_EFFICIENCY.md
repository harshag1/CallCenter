...
3. **Provider instruction block:** exact output of `activeCapabilityCatalogInstructions(catalog)`, including the full production catalog plus its 777-byte instruction wrapper in this corpus.
...
| Provider instruction block | No valid raw-full instruction block | 11,733 min; 12,140 median; 12,479 max | Exact production block size |
| T4 for provider instruction block | Not comparable | 2,934 min; 3,035.5 median; 3,120 max | Absolute estimate only |
...
|---|---|---:|---:|---:|
| Identity | `check_identity_match`, `create_identity_review`, `inspect_consent_status`, `list_verification_methods`, `lock_customer_profile`, `lookup_customer_profile`, `record_verification`, `update_contact_details` | 8,075 / 2,019 | 10,956 / 2,739 | 11,733 / 2,934 |
| Membership | `change_membership_plan`, `check_membership_benefits`, `list_membership_plans`, `lookup_membership`, `pause_membership`, `quote_membership_renewal`, `renew_membership`, `send_membership_receipt` | 8,009 / 2,003 | 11,005 / 2,752 | 11,782 / 2,946 |
| Billing | `collect_payment`, `create_payment_plan`, `explain_charge`, `list_recent_charges`, `lookup_invoice`, `quote_payment_plan`, `send_billing_receipt`, `update_payment_method` | 7,944 / 1,986 | 11,071 / 2,768 | 11,848 / 2,962 |
| Orders | `cancel_order`, `check_fulfillment_status`, `list_order_items`, `locate_order`, `quote_delivery_window`, `reroute_shipment`, `send_tracking_update`, `update_shipping_address` | 7,978 / 1,995 | 11,280 / 2,820 | 12,057 / 3,015 |
| Returns | `check_return_eligibility`, `create_return`, `estimate_refund`, `issue_return_label`, `list_return_methods`, `lookup_return_policy`, `schedule_return_pickup`, `send_return_confirmation` | 7,989 / 1,998 | 11,446 / 2,862 | 12,223 / 3,056 |
| Scheduling | `book_appointment`, `cancel_appointment`, `check_availability`, `inspect_provider_constraints`, `list_appointments`, `quote_appointment_options`, `reschedule_appointment`, `send_appointment_reminder` | 8,021 / 2,006 | 11,526 / 2,882 | 12,303 / 3,076 |
| Communications | `check_callback_window`, `inspect_delivery_status`, `list_supported_languages`, `schedule_callback`, `send_email`, `send_sms`, `transfer_call`, `verify_contact_channel` | 7,962 / 1,991 | 11,595 / 2,899 | 12,372 / 3,093 |
| Escalation | `add_case_note`, `check_service_health`, `create_support_case`, `handoff_to_human`, `inspect_case_history`, `list_escalation_queues`, `page_on_call_specialist`, `search_knowledge` | 7,989 / 1,998 | 11,702 / 2,926 | 12,479 / 3,120 |
