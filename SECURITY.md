...
- Builder inference, onboarding AI/research, and browser realtime sessions use deployment provider credentials. They fail closed in production because this release has neither a tenant-scoped BYOK read path nor durable provider-budget authority. `ALLOW_DEV_DEPLOYMENT_FUNDED_AI=true` is accepted only outside production with a plain-HTTP loopback `PUBLIC_ORIGIN`, for a developer spending their own key on their own machine; copying it to a tunnel or deployment does not enable these routes. Deterministic onboarding remains available without AI egress.
...
- Never paste env values or external-MCP Authorization into builder chat. The authenticated same-origin `/api/credentials/ingest` workflow returns an opaque one-time reference for external tool credentials; it is not a tenant-BYOK path for realtime or builder-model keys. The stock UI does not yet include a general credential form.
