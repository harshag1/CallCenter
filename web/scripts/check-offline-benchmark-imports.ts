...
const entry = "scripts/voice-benchmark-offline.ts";
...
async function main(): Promise<void> {
  ...
  if (!result.metafile) throw new Error("esbuild did not return an offline import graph");
  const runtimeInputs = Object.keys(result.metafile.inputs).sort();
  const forbiddenRuntimeInputs = runtimeInputs.filter((path) => (
    path.endsWith("/lib/benchmark/paid-runner.ts")
    || path === "lib/benchmark/paid-runner.ts"
