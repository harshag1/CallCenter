...
import {
  ...
  type OperatorTurnInferenceAuthority,
...
export type BuilderModelOptions = {
  /** One host-created authority must span every sample and nested research
   * request belonging to the authenticated operator turn. */
...
const MAX_TOOL_CALLS = OPERATOR_TURN_INFERENCE_LIMITS.maxToolCalls;
...
function safeProviderFailure(provider: VoiceProviderId, response: Response): Error {
  ...
  const requestId = REQUEST_ID.test(requestIdCandidate)
    ? requestIdCandidate
    : null;
  return new Error(
...
function appendBounded(
  ...
  if (encoder.encode(combined).byteLength > limit) {
...
export async function* streamBuilderModel(
  ...
  const maxTokens = options.maxTokens
    ?? OPERATOR_TURN_INFERENCE_LIMITS.maxBuilderOutputTokensPerRequest;
  const body = requestBody(messages, config, {
    ...options,
    model: requestedModel,
    maxTokens,
  });
  const lease = openOperatorBuilderRequest(options.authority, {
  ...
  try {
    const response = await lease.dispatch();
    if (!response.ok) {
      const error = safeProviderFailure(config.provider, response);
      await lease.guard(response.body?.cancel() ?? Promise.resolve())
        .catch(() => undefined);
      throw error;
    }
    if (!response.body) {
    ...
    const consumeLine = function* (line: string): Generator<BuilderModelStreamEvent> {
      ...
      if (typeof content === "string" && content) {
        emittedTextBytes += encoder.encode(content).byteLength;
    ...
    while (true) {
      const { done, value } = await lease.guard(reader.read());
      if (done) break;
    ...
    if (reader) {
      await lease.guard(reader.cancel()).catch(() => undefined);
    }
    lease.close();
  }
}
