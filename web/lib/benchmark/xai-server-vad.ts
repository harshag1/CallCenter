...
/** Exact documented xAI server-VAD policy frozen for the interactive qualification/live path. */
 ...
 * close deterministically even when the fixture itself ends on speech. The
 * delimiter has a hard 2 second cap: native speech-stop may close it after the
 * configured 500 ms minimum, while exhausting the cap without a native stop is
 * an immediate transport failure rather than a later response timeout.
 */
...
export const LC4_XAI_SERVER_VAD_TRANSPORT_DISCLOSURE_SHA256 = sha256Hex(
  `harshas-amazing-call-center/lc4-xai-server-vad-disclosure/v5\n${canonicalJson(LC4_XAI_SERVER_VAD_TRANSPORT_DISCLOSURE)}`,
);
