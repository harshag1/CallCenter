...
function devGatewayCall(event: NormalizedRealtimeEvent): Readonly<Record<string, unknown>> | null {
  if (event.type === "tool.dispatch") {
    ...
    return Object.freeze({
      ...
      call_id_sha256: sha256Hex(dispatch.callId),
  ...
  return Object.freeze({
    evidence_kind: "lc4_dev_gateway_call_without_dispatch_adapter",
    provider: event.provider,
    response_id_sha256: sha256Hex(event.responseId),
    call_id_sha256: sha256Hex(call.callId),
...
export async function executeLc4DevAudioCanary(input: Input): Promise<Lc4DevAudioCanaryExecution> {
  ...
  const unsubscribeEvent = input.client.onEvent((event) => {
    ...
    const controlled = event.provider === input.provider ? devGatewayCall(event) : null;
    if (controlled) {
      callEvidence = controlled;
      responseEvidence.push(Object.freeze({
        type: "lc4_dev_gateway_tool_call_observed",
        evidence_sha256: sha256Hex(canonicalJson(controlled)),
      }));
      finish?.();
  ...
  return Object.freeze({
    ...
    providerToolCallEvidenceSha256: callEvidence === null
      ? null
      : sha256Hex(`harshas-amazing-call-center/lc4-dev-audio-tool-call-evidence/v1\n${canonicalJson(callEvidence)}`),
