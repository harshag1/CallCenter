...
export function renderLongHorizonManifest(): string {
  ...
  const lines = [
    ...
    "",
    `The generator replayed ${totals.calls} oracle calls into ${totals.receipts} receipts, ${totals.effects} authoritative effects, and ${totals.events} bound events across all nine fixtures. Every expected receipt status matched; every oracle reached \`task_success=true\` with zero failed declared assertions; and every six-arm parity audit passed. This proves only that the synthetic fixtures have coherent safe paths. It says nothing about model or harness performance.`,
    "",
