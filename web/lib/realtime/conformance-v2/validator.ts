...
type ToolCallState = { turnId: string; responseId: string; resultId?: string; acknowledged: boolean };
type UsageState = {
...
export class RealtimeLifecycleConformanceValidator {
  ...
  private acknowledgeConfiguration(index: number, event: Extract<RealtimeLifecycleEvent, { type: "session.configuration.acknowledged" }>): void {
    ...
    if (!validId(event.providerSessionId) || !this.providerEvidence(index, event, event.evidence)
  ...
  private startTurn(index: number, event: Extract<RealtimeLifecycleEvent, { type: "turn.started" }>): void {
    ...
    if (event.evidence && !this.providerEvidence(index, event, event.evidence)) return;
  ...
  private startResponse(index: number, event: Extract<RealtimeLifecycleEvent, { type: "response.started" }>): void {
    if (!this.requireActive(index, event) || !this.openTurn(index, event, event.turnId)
      || !validId(event.responseId) || !this.providerEvidence(index, event, event.evidence)) return;
  ...
  private recordAudio(index: number, event: Extract<RealtimeLifecycleEvent, { type: "response.audio.delta" }>): void {
    ...
    if (!response || !this.providerEvidence(index, event, event.evidence)) return;
  ...
  private completeToolCall(index: number, event: Extract<RealtimeLifecycleEvent, { type: "tool.call.completed" }>): void {
    if (!this.openResponse(index, event, event.turnId, event.responseId)
      ...
      || !validSha256(event.argumentsSha256) || !this.providerEvidence(index, event, event.evidence)) {
  ...
  private providerEvidence(index: number, event: RealtimeLifecycleEvent, evidence: LifecycleEvidence): evidence is Extract<LifecycleEvidence, { authority: "provider_wire" }> {
    ...
    if (!validId(evidence.providerEventId) || !validId(evidence.wireType) || !Number.isFinite(evidence.observedAtMs)) {
      this.violate(index, event, "invalid_event", "provider acknowledgement evidence is incomplete");
      return false;
    }
