...
describe("browser voice-session capability rotation wiring", () => {
  beforeEach(() => {
    ...
    mocks.activeCapabilityAuthorityFor.mockResolvedValue({
      catalog: {
        schema_version: 1,
        availability: "active",
        runtime_digest: callRuntimeDigest(snapshot),
  ...
  it("issues distinct generation-zero MCP/renewal capabilities and the exact local renewal contract", async () => {
    const spec = await voiceSessionSpecForCall(agent, CALL_ID, "web", ORIGIN);
    ...
    expect(mocks.activeCapabilityAuthorityFor).toHaveBeenCalledWith({
      callId: CALL_ID,
      agentId: AGENT_ID,
      orgId: ORG_ID,
    });
