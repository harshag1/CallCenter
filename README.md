...
- A provider-neutral, non-generating conversation-history hydration contract for long-call rotation. It preserves caller-heard user/assistant turns plus ordered parallel tool-call batches, keeps source bindings separate from provider-visible content, and fails before new audio when any item, acknowledgement, or wire-order proof is missing.
