...
The command initializes an empty local cluster, applies all 31 numbered migrations (`001`–`031`) as a non-super migration owner, reapplies the upgrade range to test idempotency, runs the inventory-bound 18 conditional database suites/54 tests with zero skips, and deletes the cluster afterward. It verifies every `public` and `hacc_private` relation has enabled and forced RLS plus explicit backend and migration-owner policies. It also exercises public API CRUD denials, runtime grants, worker data denial, concurrent `SKIP LOCKED` claims, irreversible dispatch/completion state, and the recording-deletion ledger. It opens no provider session and spends `$0`.
