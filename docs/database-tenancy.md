...
`DATABASE_URL` must identify `hacc_runtime` in production. Set `DATABASE_RUNTIME_ROLE=hacc_runtime`; application startup verifies the exact connected role, membership, ownership, superuser, and RLS-bypass properties. The scheduler should run with its own worker connection when deployed as a separate process.
...
The command initializes an empty local cluster, applies all 36 numbered migrations (`001`–`036`) as a non-super migration owner, reapplies migrations `016`–`036` to test idempotency, runs the inventory-bound conditional database suites with zero skips, and deletes the cluster afterward. The exact suite/test counts come from [`GATE0_SKIP_INVENTORY.json`](../benchmarks/voice-long-horizon/GATE0_SKIP_INVENTORY.json), whose per-file hashes are checked before the suites run. The proof verifies every `public` and `hacc_private` relation has enabled and forced RLS plus explicit backend and migration-owner policies. It also exercises public API CRUD denials, runtime grants, worker data denial, concurrent `SKIP LOCKED` claims, irreversible dispatch/completion state, and the recording-deletion ledger. It opens no provider session and spends `$0`.
