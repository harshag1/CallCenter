...
Status: design proposal informed by the retained HACC-LC3-v6 development run. The repository contains several required safety and durability primitives, but the pushed response plan, flow-bound invocation, audio-bound slot extractor, bounded same-turn repair controller, and automatic worker orchestration described here are **not implemented in the live provider path** and have not demonstrated a HACC efficacy advantage.
...
|---|---|---|
| Flow state | Flow v2 persists checkpoints, capability epochs, step attempts, typed outputs, and receipt-backed transitions. The v6 benchmark requires a model-authored `flow.get_state` call at each caller-turn boundary. | The host pushes a complete, revision-bound response plan before generation and after every authority-changing event. Routine state polling disappears; `flow.get_state` remains only for reconnect or explicit recovery. |
